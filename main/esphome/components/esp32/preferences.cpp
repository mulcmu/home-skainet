...
class ESP32Preferences : public ESPPreferences {
 public:
  ...
  void open() {
    ESP_LOGI(TAG, "ESP32 Prefs open()");

    esp_err_t err = nvs_flash_init();
    ESP_LOGW(TAG, "nvs_flash_init(): %d  %s", err, esp_err_to_name(err));
    
    err = nvs_open("esphome", NVS_READWRITE, &nvs_handle);
    ESP_LOGW(TAG, "nvs_open(): %d  %s", err, esp_err_to_name(err));
    ...
    ESP_LOGW(TAG, "nvs_deinit(): %d  %s", err, esp_err_to_name(err));
    err = nvs_flash_erase();
    ESP_LOGW(TAG, "nvs_erase(): %d  %s", err, esp_err_to_name(err));
    err = nvs_flash_init();
    ESP_LOGW(TAG, "nvs_flash_init(): %d  %s", err, esp_err_to_name(err));
  ...
  bool sync() override {
    ...
    if (failed > 0) {
      ESP_LOGE(TAG, "Error saving %d preferences to flash. Last error=%s for key=%s", failed, esp_err_to_name(last_err),
               last_key.c_str());
      if (last_err == ESP_ERR_NVS_INVALID_HANDLE)  {
...
void setup_preferences() {
  ESP_LOGD(TAG, "setup_preferences()");
  auto *prefs = new ESP32Preferences();  // NOLINT(cppcoreguidelines-owning-memory)
  prefs->open();
  global_preferences = prefs;
}
